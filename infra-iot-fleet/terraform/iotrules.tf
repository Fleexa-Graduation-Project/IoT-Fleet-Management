...
resource "aws_iot_topic_rule" "telemetry_processor" {
  ...
  enabled     = true
  sql         = "SELECT topic() as topic, device_id, timestamp, type, payload FROM 'devices/+/telemetry'"
...
resource "aws_iot_topic_rule" "alert_processor" {
  ...
  enabled     = true
  sql         = "SELECT topic() as topic, device_id, timestamp, type, payload FROM 'devices/+/alerts'"
  sql_version = "2016-03-23"
